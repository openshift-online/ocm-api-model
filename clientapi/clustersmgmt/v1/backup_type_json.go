...
func MarshalBackup(object *Backup, writer io.Writer) error {
	stream := helpers.NewStream(writer)
	WriteBackup(object, stream)
	err := stream.Flush()
	if err != nil {
		return err
	}
...
func WriteBackup(object *Backup, stream *jsoniter.Stream) {
	...
	if present_ {
		...
		stream.WriteObjectField("backup_state")
		stream.WriteString(string(object.backupState))
	}
...
func UnmarshalBackup(source interface{}) (object *Backup, err error) {
	iterator, err := helpers.NewIterator(source)
	if err != nil {
		return
	}
	object = ReadBackup(iterator)
	err = iterator.Error
	return
}

// ReadBackup reads a value of the 'backup' type from the given iterator.
func ReadBackup(iterator *jsoniter.Iterator) *Backup {
	object := &Backup{
