...
func (l *CIDRBlockAccessList) Get(i int) *CIDRBlockAccess {
...
func (l *CIDRBlockAccessList) Slice() []*CIDRBlockAccess {
	var slice []*CIDRBlockAccess
	if l == nil {
		slice = make([]*CIDRBlockAccess, 0)
	} else {
		slice = make([]*CIDRBlockAccess, len(l.items))
		copy(slice, l.items)
	}
	return slice
}
