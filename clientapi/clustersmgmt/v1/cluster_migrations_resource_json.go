...
func writeClusterMigrationsAddRequest(request *ClusterMigrationsAddRequest, writer io.Writer) error {
	return MarshalClusterMigration(request.body, writer)
}
func readClusterMigrationsAddResponse(response *ClusterMigrationsAddResponse, reader io.Reader) error {
	var err error
...
func readClusterMigrationsListResponse(response *ClusterMigrationsListResponse, reader io.Reader) error {
	...
	for {
		...
		case "items":
			...
			response.items = &ClusterMigrationList{
				items: items,
			}
		default:
			iterator.ReadAny()
