...
func writeUpgradePoliciesAddRequest(request *UpgradePoliciesAddRequest, writer io.Writer) error {
	return MarshalUpgradePolicy(request.body, writer)
}
func readUpgradePoliciesAddResponse(response *UpgradePoliciesAddResponse, reader io.Reader) error {
	var err error
...
func readUpgradePoliciesListResponse(response *UpgradePoliciesListResponse, reader io.Reader) error {
	...
	for {
		...
		case "items":
			...
			response.items = &UpgradePolicyList{
				items: items,
			}
		default:
			iterator.ReadAny()
