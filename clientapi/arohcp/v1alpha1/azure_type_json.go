...
func WriteAzure(object *Azure, stream *jsoniter.Stream) {
	...
	present_ = len(object.fieldSet_) > 1 && object.fieldSet_[1] && object.imageRegistryCredentials != nil
	if present_ {
		if count > 0 {
			stream.WriteMore()
		}
		stream.WriteObjectField("image_registry_credentials")
		WriteAzureImageRegistryCredentials(object.imageRegistryCredentials, stream)
		count++
	}
	...
	if present_ {
		...
		stream.WriteObjectField("oidc_issuer_url")
...
func ReadAzure(iterator *jsoniter.Iterator) *Azure {
	...
	for {
		...
		case "image_registry_credentials":
			value := ReadAzureImageRegistryCredentials(iterator)
			object.imageRegistryCredentials = value
