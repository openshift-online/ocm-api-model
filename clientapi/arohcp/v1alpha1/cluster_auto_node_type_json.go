...
func WriteClusterAutoNode(object *ClusterAutoNode, stream *jsoniter.Stream) {
	...
	if present_ {
		...
		WriteAwsAutoNodeConfiguration(object.aws, stream)
		count++
	}
	...
	if present_ {
		...
		stream.WriteString(string(object.mode))
		count++
	}
	present_ = len(object.fieldSet_) > 2 && object.fieldSet_[2] && object.status != nil
	if present_ {
		if count > 0 {
			stream.WriteMore()
		}
...
func ReadClusterAutoNode(iterator *jsoniter.Iterator) *ClusterAutoNode {
	...
	for {
		...
		switch field {
		case "aws":
			value := ReadAwsAutoNodeConfiguration(iterator)
			object.aws = value
			object.fieldSet_[0] = true
		case "mode":
			text := iterator.ReadString()
			value := AutoNodeMode(text)
			object.mode = value
